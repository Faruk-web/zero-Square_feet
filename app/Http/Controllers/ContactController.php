...
class ContactController extends Controller
{
    ...
    public function emailForUser(Request $request){
        ...
        MailHelper::setMailConfig();
            $poperty = DB::table('properties')->where('id',$request->poperty_id)->first();
        $template=EmailTemplate::where('id',2)->first();
        $message=$request->message . '<br>' . 'User Contact Info:' . '<br>' . $request->name . '<br>' . $request->phone . '<br>' . $request->email ; 
       
        $subject=$poperty->title;
